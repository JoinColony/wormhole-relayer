...
export const processNextBlock = async (chainId: string): Promise<void> => {
  if (isProcessing[chainId]) {
    return;
  }

  // Only allow one instance of the function to run at any given time
  isProcessing[chainId] = true;

  let lastBlockNumber = getLastBlockNumber(chainId);
  ...
  while (blocksMap.get(lastBlockNumber + 1)) {
    ...
    setLastBlockNumber(chainId, currentBlockNumber);
  }

  isProcessing[chainId] = false;
};
